 ...
 * @return mixed
 */
function rambo_get_prevdem_img_src( $i = 0 ) {
	// prevent infinite loop
	if ( 10 == $i ) {
		return '';
	}

	$path = get_template_directory() . '/rambo-demo-image/img/';
	...
	if ( ! file_exists( $path . $img_name ) ) {
		...
		$i++;
		return rambo_get_prevdem_img_src( $i );
	}
	...
	return get_template_directory_uri() . '/rambo-demo-image/img/' . $img_name;
}
 ...
 */
function rambo_preview_the_post_thumbnail( $input ) {
	if ( empty( $input ) ) {
		$placeholder = rambo_get_prevdem_img_src();
		return '<img src="' . esc_url( $placeholder ) . '" class="blog_section2_img">';
	}
	return $input;
}

add_filter( 'post_thumbnail_html', 'rambo_preview_the_post_thumbnail' );
